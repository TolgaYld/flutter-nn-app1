...
class EditStoreScreen extends StatefulWidget {
  ...
  const EditStoreScreen({Key? key, required this.addressId}) : super(key: key);
...
//Get LatLong
  ...
  late String? _selectedRubricId;
  late String? _selectedSubrubricId;
    ...
    if (isInit) {
      ...
      InvoiceAddress invoiceAddress =
          Provider.of<InvoiceAddresses>(context, listen: false)
              .findInvoiceAddressFromAddress(addressId)!;
      Category category = Provider.of<Categorys>(context, listen: false)
          .findById(address.categoryId);
      Subcategory subcategory =
          Provider.of<Subcategorys>(context, listen: false)
              .findById(address.subcategoryId);

      _selectedRubricId = address.categoryId;
      this._selectedSubrubricId = address.subcategoryId;
      subcategorys = Provider.of<Subcategorys>(context, listen: false)
          .findByCategoryId(_selectedRubricId!);

      _rubricName = category.name;
      _subrubricName = subcategory.name;
      subrubricActive = true;

      if (address.subsubcategoryId != null) {
        Subsubcategory subsubcategory =
            Provider.of<Subsubcategorys>(context, listen: false)
                .findById(address.subsubcategoryId!);

        _selectedSubSubrubricId = subsubcategory.id;
        _subsubrubricName = subsubcategory.name;
        subsubcategorys = Provider.of<Subsubcategorys>(context, listen: false)
            .findByCategoryId(_selectedSubSubrubricId!);
        _subrubricValidate = true;
      }
      ...
      _ibanController.text = address.iban == null ? "" : address.iban!;
      _countryController.text = address.country;
      _homepageController.text =
          address.homepage != null ? address.homepage! : "";
      _facebookController.text =
          address.facebook != null ? address.facebook! : "";
      _instagramController.text =
          address.instagram != null ? address.instagram! : "";
      _youtubeController.text = address.youtube != null ? address.youtube! : "";
      _tiktokController.text = address.tiktok != null ? address.tiktok! : "";
      _googleMybusinessController.text =
          address.googleMyBusiness != null ? address.googleMyBusiness! : "";
      _pinterestController.text =
          address.pinterest != null ? address.pinterest! : "";
      _invoiceLat = invoiceAddress.latitude;
      _invoiceLng = invoiceAddress.longitude;
      _invoiceCountryCode = invoiceAddress.countryCode;
      _invoiceGender = invoiceAddress.gender;
      _invoiceFirstnameController.text = invoiceAddress.firstname;
      _invoiceLastnameController.text = invoiceAddress.lastname;
      _invoiceEmailController.text = invoiceAddress.email;
      _invoiceStreetController.text = invoiceAddress.street;
      _invoiceFloorController.text =
          invoiceAddress.floor != null ? invoiceAddress.floor! : "";
      _invoicePostCodeController.text =
          invoiceAddress.postcode != null ? invoiceAddress.postcode! : "";
      _invoiceCityController.text = invoiceAddress.city;
      _invoiceCountryController.text = invoiceAddress.country;
      _invoicePhoneController.text =
          invoiceAddress.phone != null ? invoiceAddress.phone! : "";
      _taxIdController.text = address.vat!;
      _storePhoneController.text = address.phone != null ? address.phone! : "";

      _calculateAndGetOpeningHours();
  ...
  _selectTakePic(BuildContext context) async {
    await showAdaptiveActionSheet(
      ...
      actions: <BottomSheetAction>[
        ...
        if (_imageFile != null || address.media!.isNotEmpty)
          BottomSheetAction(
              title: Text(
                "Delete",
                style: TextStyle(
  ...
  Widget _buildStoreCategoryPage(BuildContext context) {
    ...
    List<Category> categorys =
        Provider.of<Categorys>(context, listen: false).categorys;
    Widget _emptySpaceColum =
        SizedBox(height: MediaQuery.of(context).size.height * 0.03);
    Widget _emptySpaceColumTextField =
        SizedBox(height: MediaQuery.of(context).size.height * 0.02);
                                        ...
                                        if (_subrubricSuggestion.text.length >
                                          ...
                                          Subcategory suggestion =
                                              await Provider.of<Subcategorys>(
                                                  ...
                                                  .addSuggestion(
                                            Subcategory(
                                                name: _subrubricSuggestion.text,
  ...
  Future uploadImageAndCategory() async {
    if ((_imageFile == null && address.media!.isEmpty) ||
        _selectedRubricId == null ||
        _selectedSubrubricId == null ||
  ...
  Future getStarted() async {
    try {
      await Provider.of<Addresses>(context, listen: false).updateAddress(
        openingHours: _openingHoursList,
        invoiceAddress: InvoiceAddress(
          id: invoiceAddress!.id!,
          floor: _invoiceFloorController.text,
        ...
        address: Address(
          ...
          media: address.media,
          qrofferShortDescription: address.qrofferShortDescription,
        ),
      );
      Provider.of<OpeningHours>(context, listen: false)
          .deleteOpeningHourWhere(address.id!);
  ...
  void _calculateAndGetOpeningHours() {
    _openingHoursListDateTimeRange.clear();
    for (var i = 0; i < _openingHours.length; i++) {
      _openingHoursList.add({
        "day": _openingHours[i].day,
        "day_from": _openingHours[i].dayFrom,
        "day_to": _openingHours[i].dayTo,
        "time_from": formatTimeOfDay(_openingHours[i].timeFrom),
        "time_to_duration": _openingHours[i].duration,
      });
      ...
      for (var j = 0; j < 7; j++) {
        if (helper.add(Duration(days: j)).weekday == _openingHours[i].day) {
          ...
          _openingHoursListDateTimeRange.add({
            "day": _openingHours[i].day,
            "datetimerange": DateTimeRange(start: begin, end: end),
          });
        }
      }
    }
