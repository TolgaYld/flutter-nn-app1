...
class EditStadScreen extends StatefulWidget {
  String stadId;
  EditStadScreen({Key? key, required this.stadId}) : super(key: key);

  static const routeName = '/edit_stad';
  @override
  _EditStadScreenState createState() => _EditStadScreenState();
}

class _EditStadScreenState extends State<EditStadScreen> {
  late String stadId;
  ...
  Future<CameraPosition> animateCameraToNextStore() async {
    return await CameraPosition(
  ...
  Future<void> _goToNextStore() async {
    ...
    await controller.animateCamera(
        CameraUpdate.newCameraPosition(await animateCameraToNextStore()));
  }
  ...
  Future<void> _submit() async {
    ...
    if (_shortDescriptionController.text.length < 3 ||
      ...
      if (_files.isEmpty == null) {
        errorDetailText = errorDetailText +
            "\n- Picture: Select or take minimum one image or video file.";
        setState(() {
          fileBorderColor = Colors.red;
        });
      }
      ...
      if (_files.isNotEmpty) {
        _imgOrVidUrls = _files;
        // if (_file2 != null || _file3 != null || _file4 != null || _file5 != null || _file6 != null) {

        for (int i = 0; i < _files.length; i++) {
          if (_files[i] is File) {
            int index = i;
            Dio _dio = Dio();
            String mimeStr = lookupMimeType(_files[i].path)!;

            var fileType = mimeStr.split('/');

            FormData _formData = FormData.fromMap({
              "file": await MultipartFile.fromFile(_files[i].path),
              "upload_preset": Enviroment.cloudinaryUploadPreset,
              "cloud_name": Enviroment.cloudinaryCloudName,
            });
            if (fileType.contains("video")) {
              try {
                Response _responseImgOrVid = await _dio.post(
                  Enviroment.videoApi,
                  data: _formData,
                );

                if (_responseImgOrVid != null) {
                  var _data = await jsonDecode(_responseImgOrVid.toString());
                  if (_data != null) {
                    _files.removeAt(index);
                    _files.insert(index, _data['secure_url'].toString().trim());
                  }
                }
              } catch (e) {
        ...
        if (_files.length == _imgOrVidUrls.length &&
            _imgOrVidUrls.isNotEmpty &&
            parsedPrice != null) {
          ...
          if (!hasException) {
            try {
              await Provider.of<Stads>(context, listen: false).updateStad(
                id: stadId,
                stad: Stad(
  ...
  GlobalKey _timeToKey = GlobalKey();
  bool _isInit = true;
        ...
        Provider.of<Subcategorys>(context, listen: false)
          ...
          Provider.of<Subsubcategorys>(context, listen: false)
            ...
            Provider.of<Addresses>(context, listen: false)
                .fetchAllMyAddresses()
                .then((_) {
              Provider.of<SubsubCategorysStad>(context, listen: false)
                  .fetchAllSubsubcategorys(widget.stadId)
                  .then((value) {
                checkboxListSubSubRubric.clear();
                _selectedSubsubcategorys.clear();

                _selectedSubsubcategorys =
                    Provider.of<SubsubCategorysStad>(context, listen: false)
                        .subsubcategorys;

                for (var i = 0; i < _selectedSubsubcategorys.length; i++) {
                  pickedCheckboxIds
                      .add(_selectedSubsubcategorys[i].subsubcategoryId);
                }

                stadId = widget.stadId;
                stad =
                    Provider.of<Stads>(context, listen: false).findById(stadId);
                _address = Provider.of<Addresses>(context, listen: false)
                    .findById(stad.addressId);
                ...
                if (_subsubcategorys.length == pickedCheckboxIds.length) {
                ...
                Provider.of<OpeningHours>(context, listen: false)
                  ...
                  if (_openingHours.isEmpty) {
                    ...
                    minimumTimeTo();
                    maximumTimeTo();
                    var timezone = tz.getLocation(_address.timezone!.first);
                    _setMinTimeFrom = tz.TZDateTime.from(stad.begin!, timezone);
                    _setMinTimeTo = tz.TZDateTime.from(stad.end!, timezone);
                    DateTime? start = _setMinTimeFrom;
                    DateTime? end = _setMinTimeTo;
                    ...
                    _hasFlatrateMethod();
                    initialDates();
                    _shortDescriptionController.text = stad.shortDescription;
                    _longDescriptionController.text = stad.longDescription;
                    _files = stad.media;
                    price = stad.price;
                    _notificationMeters = stad.pushNotificationRadius;
                    _visibleMeters = stad.displayRadius;
                  }
  ...
  late Stad stad;
  List<SubsubCategoryStad> _selectedSubsubcategorys = [];
                            ...
                            onPressed: () async {
                              ...
                              await showDialog(
                                  ...
                                  builder: (BuildContext context) {
                                    ...
                                    Stad? stad = Provider.of<Stads>(context,
                                            listen: false)
                                        .activeStad(_address.id!);
                                    ...
                                    for (var i = 0;
                                        i < openingHoursAndDays.length;
                                        i++) {
                                      if (_dateTimeRange!.start.weekday ==
                                              openingHoursAndDays[i]["day"] ||
                                          _dateTimeRange!.start.weekday ==
                                              openingHoursAndDays[i]
                                                  ["day_from"] ||
                                          _dateTimeRange!.start.weekday ==
                                              openingHoursAndDays[i]
                                                  ["day_to"]) {
                                        if (_dateTimeRange!.start
                                                .isAtSameMomentAs(
                                                    openingHoursAndDays[i]
                                                        ["datetime_from"]) ||
                                            _dateTimeRange!.start.isAfter(
                                                openingHoursAndDays[i]
                                                    ["datetime_from"]) ||
                                            _dateTimeRange!.start.isBefore(
                                                openingHoursAndDays[i]
                                                    ["datetime_to"])) {
                                          List filteredList =
                                              openingHoursAndDays
                                          ...
                                          filteredList.sort((obj1, obj2) =>
                                              compareDate(obj1["datetime_from"],
                                                  obj2["datetime_from"]));

                                          tillDate =
                                              filteredList.last["datetime_to"];
                                        }
                                      }
                                    }
                                      ...
                                      Padding(
                                        ...
                                        child: Center(
                                          child: Column(
                                            children: [
                                              Text(
                                                  "Zu welchen Unterkategorien passt dein STAD?"),
                                              SizedBox(
                                                height: _height * 0.03,
                                              ),
                                              Container(
  ...
  _videoPreview(BuildContext context) {
    double _width = MediaQuery.of(context).size.width;
    double _height = MediaQuery.of(context).size.height;
    return Material(
          ...
          child: Column(
            children: [
                ...
                children: [
                  IconButton(
                      ...
                      onPressed: () {
                        ...
                        if (_file2 != null) {
                          String mimeStr = lookupMimeType(_file2!.path)!;
                          var fileType = mimeStr.split('/');
                          if (fileType.contains("video")) {
                            setState(() {
                              _file2 = null;
                        ...
                        if (_file3 != null) {
                          String mimeStr = lookupMimeType(_file3!.path)!;
                          var fileType = mimeStr.split('/');
                          if (fileType.contains("video")) {
                            setState(() {
                              _file3 = null;
                        ...
                        if (_file4 != null) {
                          String mimeStr = lookupMimeType(_file4!.path)!;
                          var fileType = mimeStr.split('/');
                          if (fileType.contains("video")) {
                            setState(() {
                              _file4 = null;
                        ...
                        if (_file5 != null) {
                          String mimeStr = lookupMimeType(_file5!.path)!;
                          var fileType = mimeStr.split('/');
                          if (fileType.contains("video")) {
                            setState(() {
                              _file5 = null;
                        ...
                        if (_file6 != null) {
                          String mimeStr = lookupMimeType(_file6!.path)!;
                          var fileType = mimeStr.split('/');
                          if (fileType.contains("video")) {
                            setState(() {
                              _file6 = null;
              ...
              ClipRRect(
                ...
                child: VideoPlayerWidget(_previewFile),
              ),
            ],
          ),
        ),
                              ...
                              Padding(
                                ...
                                child: Container(
                                  decoration: const BoxDecoration(
                                      ...
                                      color:
                                          Colors.purple // TODO rubric color!!!,
                                      ),
                                  child: Padding(
                                    padding: EdgeInsets.only(
                                        bottom:
...
//
  ...
  Container _imageContainerWithFile(BuildContext context, var file) {
    double _width = MediaQuery.of(context).size.width;
  ...
  Widget _checkPictureOrVideo(var file) {
    var fileType;
    if (file is File) {
      if (file != null) {
    ...
    if (fileType.contains("image")) {
      return Container(
        decoration: BoxDecoration(
          image: DecorationImage(fit: BoxFit.cover, image: NetworkImage(file!)),
          borderRadius: const BorderRadius.all(
            Radius.circular(100),
          ),
        ),
      ...
      return ClipRRect(
        borderRadius: BorderRadius.circular(100),
        child: NetworkPlayerWidget(url: file!),
      );
    }
  }

  _onPressedOnSelectPhoto(var file) async {
    ...
    if (_files.isNotEmpty) {
      if (_files[_files.length - 1] != null) {
        String mimeStr = _files.first is File
            ? lookupMimeType(_files.first.path)!
            : lookupMimeType(_files.first)!;
        var fileType = mimeStr.split('/');
    ...
    if (_files.isNotEmpty) {
      if (file != null) {
        mimeStr =
            file is File ? lookupMimeType(file.path) : lookupMimeType(file);
        var fileType = mimeStr!.split('/');
        if (fileType.contains("video")) {
          setState(() {
  ...
  Widget buildCheckBox(CheckboxState checkbox) {
    return Theme(
      ...
      child: CheckboxListTile(
        value: pickedCheckboxIds.contains(checkbox.id),
        controlAffinity: ListTileControlAffinity.platform,
        checkColor: Colors.white,
        activeColor: Theme.of(context).primaryColor,
        title: Text(checkbox.title!),
  ...
  Widget _media(File? file) {
    if (file != null) {
      String mimeStr = lookupMimeType(file.path)!;

      var fileType = mimeStr.split('/');
      if (fileType.contains("video")) {
        return VideoWidget(file);
      } else {
        return Image.file(
          file,
    ...
    return Image.asset(
      "assets/images/advertiserappicon.png",
      width: double.infinity,
      height: MediaQuery.of(context).size.width / 4 * 3,
      fit: BoxFit.cover,
    );
