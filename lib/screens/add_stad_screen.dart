...
class AddStadScreen extends StatefulWidget {
  const AddStadScreen({Key? key}) : super(key: key);

  static const routeName = '/add_stad';
  @override
  _AddStadScreenState createState() => _AddStadScreenState();
}

class _AddStadScreenState extends State<AddStadScreen> {
  ...
  List calculatedDatesAndWeekdays = [];
  ...
  Future<void> _submit() async {
    ...
    if (_shortDescriptionController.text.length < 3 ||
      ...
      if (_file1 != null) {
        ...
        List<String> imagesList = [_file1!.path];
        if (_file2 != null) {
          imagesList.add(_file2!.path);
        }
        if (_file3 != null) {
          imagesList.add(_file3!.path);
        }
        if (_file4 != null) {
          imagesList.add(_file4!.path);
        }
        if (_file5 != null) {
          imagesList.add(_file5!.path);
        }
        if (_file6 != null) {
          imagesList.add(_file6!.path);
        }

        for (int i = 0; i < imagesList.length; i++) {
          Dio _dio = Dio();
          String mimeStr = lookupMimeType(imagesList[i])!;

          var fileType = mimeStr.split('/');

          FormData _formData = FormData.fromMap({
            "file": await MultipartFile.fromFile(imagesList[i]),
          ...
          if (fileType.contains("video")) {
            try {
              ...
              if (_responseImgOrVid != null) {
                ...
                if (_data != null) {
                  _imgOrVidUrls.add(_data['secure_url'].toString().trim());
                }
              }
            } catch (e) {
        ...
        if (imagesList.length == _imgOrVidUrls.length &&
          ...
          if (!hasException) {
            try {
              ...
              await Alert(
                context: context,
                type: AlertType.error,
                title: "STAD could not advertised",
                desc: e.toString(),
                buttons: [
                  DialogButton(
                    child: const Text(
