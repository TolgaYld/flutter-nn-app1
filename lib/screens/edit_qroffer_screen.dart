...
class EditQrofferScreen extends StatefulWidget {
  String qrofferId;
  EditQrofferScreen({Key? key, required this.qrofferId}) : super(key: key);

  static const routeName = '/edit_qroffer';
  @override
  _EditQrofferScreenState createState() => _EditQrofferScreenState();
}

class _EditQrofferScreenState extends State<EditQrofferScreen> {
  late String qrofferId;
  ...
  Future<void> _submit() async {
    ...
    if (_shortDescriptionController.text.length < 3 ||
      ...
      if (parsedPrice != null) {
        ...
        if (!hasException) {
          try {
            await Provider.of<Qroffers>(context, listen: false).updateQroffer(
              id: qrofferId,
              qroffer: Qroffer(
            ...
            await Alert(
              context: context,
              type: AlertType.error,
              title: "QROFFER could not edited",
              desc: e.toString(),
              buttons: [
                DialogButton(
                  child: const Text(
        ...
        Provider.of<Subcategorys>(context, listen: false)
          ...
          Provider.of<Subsubcategorys>(context, listen: false)
            ...
            Provider.of<Addresses>(context, listen: false)
                .fetchAllMyAddresses()
                .then((_) {
              Provider.of<SubsubCategorysQroffer>(context, listen: false)
                  .fetchAllSubsubcategorys(widget.qrofferId)
                  .then((value) {
                checkboxListSubSubRubric.clear();
                ...
                _selectedSubsubcategorys =
                    Provider.of<SubsubCategorysQroffer>(context, listen: false)
                        .subsubcategorys;
                print(_selectedSubsubcategorys.length);
                for (var i = 0; i < _selectedSubsubcategorys.length; i++) {
                  pickedCheckboxIds
                      .add(_selectedSubsubcategorys[i].subsubcategoryId);
                }
                qrofferId = widget.qrofferId;
                qroffer = Provider.of<Qroffers>(context, listen: false)
                    .findById(qrofferId);
                ...
                Provider.of<OpeningHours>(context, listen: false)
                  ...
                  if (_openingHours.isEmpty) {
                    ...
                    _setMinTimeFrom =
                        tz.TZDateTime.from(qroffer.begin!, timezone);
                    _setMinTimeTo = tz.TZDateTime.from(qroffer.end!, timezone);
                    ...
                    _hasFlatrateMethod();
                    initialDates();
                    _shortDescriptionController.text = qroffer.shortDescription;
                    _longDescriptionController.text = qroffer.longDescription;
                    price = qroffer.price;
                    _notificationMeters = qroffer.pushNotificationRadius;
                    _visibleMeters = qroffer.displayRadius;

                    _expiryDate = tz.TZDateTime(
                      timezone,
                      qroffer.expiryDate!.year,
                      qroffer.expiryDate!.month,
                      qroffer.expiryDate!.day,
                    );
  ...
  late Qroffer qroffer;
  List<SubsubCategoryQroffer> _selectedSubsubcategorys = [];
