...
class BackendService {
  static Future<List> getPlaces(String input) async {
    var delayed = await Future.delayed(Duration(milliseconds: 777));
    print(delayed);
    try {
      String baseUrl =
      ...
      if (response.statusCode == 200) {
        List predictions = await response.data['predictions'];

        return await List.generate(predictions.length, (index) {
          return {
            'name': predictions[index]['description'],
            'street_and_number': predictions[index]['structured_formatting']
                ['main_text'],
